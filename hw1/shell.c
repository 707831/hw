  ...
  while (index < MAXTOKS && arg[index]) {
    ...
    close(1);
		...
		if(ret < 0) {
		  ...
		  fprintf(stderr, "Failed to invoke for stdout, return code is %d\n", ret);
		  return -1;
		...
		if (ret < 0) {
		  ...
		  fprintf(stderr, "Failed to invoke for stdin, return code is %d\n", ret);
		  return -1;
...
int cmd_help(tok_t arg[]) {
  ...
  for (j = 0; j < (sizeof(cmd_table) / sizeof(fun_desc_t)); j++) {
    
    printf("%s - %s\n", cmd_table[j].cmd, cmd_table[j].doc);
...
int lookup(char cmd[]) {
  ...
  for (k = 0; k < (sizeof(cmd_table) / sizeof(fun_desc_t)); k++) {
    
    if (cmd && (strcmp(cmd_table[k].cmd, cmd) == 0)) {
